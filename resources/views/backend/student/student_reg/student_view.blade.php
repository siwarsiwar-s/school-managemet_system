                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $value->student->roll ?? 'N/A' }}</td>
                            <td>{{ $value->student_year->name ?? 'N/A' }}</td>
                            <td>{{ $value->student_class->name ?? 'N/A' }}</td>
                            <td>
                                <img src="{{ (!empty($value->student->image)) ? url('upload/student_images/'.$value->student->image) : url('upload/no_image.jpg') }}" style="width:50px;">
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                      ...
                      <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $value['student_year']['name'] }}</td>
                               ...
                               <td>
                                <img  src="{{(!empty($value['student']['image']))? url(
                                'upload/student_images/'.$value['student']['image']):url('upload/no_image.jpg')}}" style="width: 60px; width:
                                60px; "></td>
 <td>  
                                <a title="Edit" href="{{route('student.registration.edit',$value->student_id)}}" class="btn btn-info"><i class="fa fa-edit"></i></a>
                                <a title="Promotion" href="{{route('student.registration.promotion',$value->student_id)}}" class="btn btn-danger" ><i class="fa fa-check"></i></a>
